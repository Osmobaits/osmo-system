...
from app.models import db, User, Role, Task
...
def manage_users():
    if request.method == 'POST':
        ...
        email = request.form.get('email') # <-- Odczytujemy email z formularza
        password = request.form.get('password')
        
        # Sprawdzamy, czy użytkownik lub email już istnieją
...
def change_password(user_id):
    ...
    if request.method == 'POST':
        ...
        # Hashujemy nowe hasło i zapisujemy w bazie
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        # Sprawdzanie unikalności nazwy użytkownika i emaila (jeśli zostały zmienione)
...
@permission_required('admin')
def all_tasks():
    # Pobieramy wszystkie zadania i sortujemy je po dacie utworzenia (od najnowszych)
